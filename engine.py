    ...
    async def connect(self, websocket: WebSocket) -> ClientConnection:
        ...
        await client.send_message({
            ...
            'data': {
                ...
                'current_weather': self.game_state.current_weather
            }
        })
        ...
        try:
            while client.active:
                try:
                    ...
                    print(f"Timeout waiting for message from {client.player_id}")
                    continue
                except WebSocketDisconnect:
                    print(f"WebSocket disconnected: {client.player_id}")
                    break
                except Exception as e:
                    print(f"Error handling client {client.player_id}: {str(e)}")
                    break
        except Exception as e:
            print(f"Critical error in handle_client for {client.player_id}: {str(e)}")
        finally:
            ...
            try:
                ...
                if self.game_state.current_weather is None:
                    if random.random() < 0.1:
                        ...
                        print(f"Weather started: {weather_type}, intensity: {intensity}, duration: {duration}")
                        await self.broadcast({
                            'type': 'weather_start',
                            'data': {
                                'type': weather_type,
...
@app.get("/")
async def root():
    return {"message": "INFINAL Server is running"}
